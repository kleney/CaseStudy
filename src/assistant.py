...
import random
import argparse
from utils.assistant_utils import load_summariser, summarise_texts, load_or_build_index
...
def ask_question(model, index, texts, years, query, top_k=TOP_K, verbose=False):
    ...
    # Arrays, with shape (number of queries, top_n)
    ...
    if verbose:
        ...
        for rank, (dist, text, year) in enumerate(results[:5], start=1):
